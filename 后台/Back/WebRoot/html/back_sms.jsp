...
<%
	...
	SmsJspResponse Sms = Utils.getSms("http://localhost:8080/Back/getSms",telNumber,keyword,startDate,endDate,pageNum,numPerPage);
	request.setAttribute("Sms",Sms.getSms());
	request.setAttribute("SmsCount",Sms.getSize());
	/**
...
<form id="pagerForm" method="post" action="back_sms.jsp">
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="back_sms.jsp" method="post" onreset="$(this).find('select.combox').comboxReset()">
	<div class="searchBar">
...
<div class="pageContent">
    ...
    <c:forEach var="sms" items="${Sms}">
		<tr target="sid_user" rel="1">
            <td>${sms.getId()}</td>
            <td>${sms.getThread_id()}</td>
            <td>${sms.getError_code()}</td>
            <td>${sms.getStatus()}</td>
            <td>${sms.getBody()}</td>
            <td>${sms.getDate()}</td>
            <td>${sms.getAddress()}</td>
        </tr>
    </c:forEach>
	...
	<div class="panelBar">
		<div class="pages">
			...
			<select class="combox" name="numPerPage" onchange="navTabPageBreak({numPerPage:this.value})">
				...
				<option value="150" <%="200".equals(numPerPage)?"selected":""%>>150</option>
			...
			<span>条，共${SmsCount}条</span>
		</div>

		<div class="pagination" targetType="navTab" totalCount="${SmsCount}" numPerPage="${numPerPage==null?50:numPerPage}" pageNumShown="10" currentPage="${pageNum==null?1:pageNum}"></div>
